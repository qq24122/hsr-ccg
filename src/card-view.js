...
export function cardImage(def) {
  if (!def?.img) return '';
  return `<img class="art" src="${encodeURI(def.img)}" alt="" loading="lazy" decoding="async">`;
}

export function staticCardHTML(def, options = {}) {
  const token = def.isToken ? ' token' : '';
  const follower = def.type === '随从';
  return `<article class="catalog-card q${esc(def.quality)}${token}" data-card-id="${esc(def.id)}" tabindex="0" style="${pathStyle(def)}">`
    + cardImage(def)
    + `<div class="cost"><b>${def.cost ?? 0}</b></div>`
    + `<div class="cname">${esc(def.name)}</div>`
    + `<div class="effect">${esc(def.note || '（无特殊效果）')}</div>`
